...
![그림](./imgs/기능추가10주차.png)

인벤토리나 설정창을 열면 마우스로 조작을 해야하기 때문에

그동안은 캐릭터를 움직일 수 없는 상태가 됩니다.

폭탄을 해체할 때도 마찬가지인데 

이를 전부 한 변수로 관리하면서 버그가 발생했습니다.

기존에는 popUp이라는 변수를 만들어 다른 화면을 조작할 때의 상태를

관리하였으나 설정창, 인벤토리, 폭탄 해체를 할 때의 변수를 각각 나누어
...
![그림](./imgs/기능추가11주차.png)

이번주 작업내용은 벽을 뚫는 버그를 고치는 것 입니다.
레이캐스트로 벽과 충돌판정을 해서 장애물이 없을 때만 이동할 수 있도록 수정하여
캐릭터가 벽을 뚫고 이동할 수 없게 했습니다.

<br>

12주차: 버그수정

![그림](./imgs/기능추가12주차.png)

저번주에 벽이 뚫리는 버그를 고치기 위해 레이캐스트로 이동방향에 장애물이 있는지 판별하도록 수정하였으나
바닥 오브젝트와 충돌해 장애물이 있다고 판단하여 이동하지 못하는 버그가 발생하여
바닥이나 계단에 floor태그를 추가하여 발밑에 있는 오브젝트일 경우에는 이동 가능하도록 수정했습니다.
벽 너머가 보이는 버그는 카메라의 위치와 범위를 조절하여 해결했습니다.

<br>

13주차: 버그수정

![그림](./imgs/기능추가13주차.png)

벽의 모서리에 캐릭터를 비비면 위로 올라가는 버그가 있었습니다.
캐릭터의 아랫방향으로 레이캐스트를 쏴서 바닥과 일정거리 이상 떨어지면 밑으로 돌려놓도록 이동 스크립트를 수정했습니다.
또한 마우스를 움직일 때 끊기는 듯한 버그가 있었으나 마우스 회전하는 스크립트를 Fixed Update에 넣어서 그랬던 것으로 확인됬습니다.
Update문으로 옮겨 해결했습니다.
